...
def add_drivetrain(V, cfg):
    ...
    if (not cfg.DONKEY_GYM) and cfg.DRIVE_TRAIN_TYPE != "MOCK":
        ...
        # elif cfg.DRIVE_TRAIN_TYPE == "DC_FOUR_WHEEL_L293D":
        #     dt = cfg.DC_FOUR_WHEEL_L293D
        #     # 定义四个电机的引脚
        #     front_left_motor = L293D(
        #         pins.pwm_pin_by_id(dt['FRONT_LEFT_FWD_PIN']),
        #         pins.pwm_pin_by_id(dt['FRONT_LEFT_BWD_PIN']))
        #     front_right_motor = L293D(
        #         pins.pwm_pin_by_id(dt['FRONT_RIGHT_FWD_PIN']),
        #         pins.pwm_pin_by_id(dt['FRONT_RIGHT_BWD_PIN']))
        #     rear_left_motor = L293D(
        #         pins.pwm_pin_by_id(dt['REAR_LEFT_FWD_PIN']),
        #         pins.pwm_pin_by_id(dt['REAR_LEFT_BWD_PIN']))
        #     rear_right_motor = L293D(
        #         pins.pwm_pin_by_id(dt['REAR_RIGHT_FWD_PIN']),
        #         pins.pwm_pin_by_id(dt['REAR_RIGHT_BWD_PIN']))
        #     # 将电机添加到车辆控制系统中
        #     V.add(front_left_motor, inputs=['front_left/throttle'])
        #     V.add(front_right_motor, inputs=['front_right/throttle'])
        #     V.add(rear_left_motor, inputs=['rear_left/throttle'])
        #     V.add(rear_right_motor, inputs=['rear_right/throttle'])


        elif cfg.DRIVE_TRAIN_TYPE == "DC_TWO_WHEEL_L298N":
            from donkeycar.parts.actuator import L293DMotor, ArduinoFirmata
            dt = cfg.DC_TWO_WHEEL_L298N
            controller = ArduinoFirmata()

            # 定义四个电机的引脚
            front_left_motor = L293DMotor(
                controller, dt['Motor4_PWM_PIN'], dt['FRONT_LEFT_BWD_PIN'], dt['Motor4_PWM_PIN'])
            
            front_right_motor = L293DMotor(
                controller, dt['Motor3_PWM_PIN'], dt['FRONT_RIGHT_BWD_PIN'], dt['Motor3_PWM_PIN'])
            
            rear_left_motor = L293DMotor(
                controller, dt['Motor1_PWM_PIN'], dt['REAR_LEFT_BWD_PIN'], dt['Motor1_PWM_PIN'])
            
            rear_right_motor = L293DMotor(
                controller, dt['Motor2_PWM_PIN'], dt['REAR_RIGHT_BWD_PIN'], dt['Motor2_PWM_PIN'])
        ...
        elif cfg.DRIVE_TRAIN_TYPE == "KEYBOARD":
            from donkeycar.parts.keyboard_controller import KeyboardController
            from donkeycar.parts.arduino_controller import ArduinoController

            # 实例化 Arduino 控制器
            keyboard_controller = KeyboardController()
            arduino_controller = ArduinoController()

            V.add(keyboard_controller, outputs=['user/command'])
            
            # 控制车辆指令
            def control_vehicle(command):
                arduino_controller.send_command(command)
            
            V.add(Lambda(control_vehicle), inputs=['user/command'])
        elif cfg.DRIVE_TRAIN_TYPE == "ARDUINO":
            #Drive train setup
