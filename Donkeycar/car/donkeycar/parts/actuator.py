...
class L293DMotor:
    def __init__(self, controller, pin_fwd, pin_bwd, pwm_pin):
        from pymata_aio.constants import Constants
        self.controller = controller
        ...
        logging.debug(f"Setting pin modes for motor: FWD_PIN={pin_fwd}, BWD_PIN={pin_bwd}, PWM_PIN={pwm_pin}")
        self.controller.set_pin_mode(self.pin_fwd, Constants.OUTPUT)
        self.controller.set_pin_mode(self.pin_bwd, Constants.OUTPUT)
        self.controller.set_pin_mode(self.pwm_pin, Constants.PWM)
    ...
    def set_speed(self, speed):
        ...
        """
        if speed is None:
            raise ValueError("Speed value cannot be None")
        
        logging.debug(f"Setting speed: {speed}")
        if speed > 0:
            self.controller.set_digital(self.pin_fwd, 1)
            self.controller.set_digital(self.pin_bwd, 0)
            self.controller.set_pwm(self.pwm_pin, int(abs(speed) * 255))
        elif speed < 0:
            self.controller.set_digital(self.pin_fwd, 0)
            self.controller.set_digital(self.pin_bwd, 1)
            self.controller.set_pwm(self.pwm_pin, int(abs(speed) * 255))
        else:
            self.controller.set_digital(self.pin_fwd, 0)
            self.controller.set_digital(self.pin_bwd, 0)
            self.controller.set_pwm(self.pwm_pin, 0)
